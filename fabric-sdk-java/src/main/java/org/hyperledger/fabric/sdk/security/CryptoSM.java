...
import org.whu.gmssl.sun.security.ec.ECPrivateKeyImpl;
...
public class CryptoSM implements CryptoSuite {
    ...
    static {
        ...
        CURVE = new Fp(SM2_ECC_P, SM2_ECC_A, SM2_ECC_B, null, null);
    ...
    public byte[] sign(PrivateKey key, byte[] plainText) throws CryptoException {
        ...
        try {
            ...
            byte[] retValue = new byte[sha256Digest.getDigestSize()];
            sha256Digest.update(plainText, 0, plainText.length);
            sha256Digest.doFinal(retValue, 0);

//            SM2Signer signer = new SM2Signer();
//            SecureRandom random = new SecureRandom();
//            ECPrivateKeyParameters priKeyParameters = getPrivateKey(key);
//            ParametersWithRandom pwr = new ParametersWithRandom(priKeyParameters, new SecureRandom());
//            signer.init(true, pwr);
//            signer.update(retValue, 0, retValue.length);
//            byte[] sig = signer.generateSignature();
            ...
            if(new String(key.getEncoded()).equals("Org1MSP")) {
                AppName = "Admin1Org1";
            } else if(new String(key.getEncoded()).equals("Org2MSP")) {
                AppName = "Admin1Org2";
            } else {
                throw new Exception("app name err");
            }
            ...
            String paramsJson = "{\"data\": \""+ Hex.toHexString(retValue) +"\",\"app_name\": \"" + AppName + "\"}";
    ...
    public boolean verify(byte[] pemCertificate, String signatureAlgorithm, byte[] signature, byte[] plainText)
        ...
        try {
            ...
            if (certificate != null) {
                ...
                if (isVerified) {
                    //TODO modify by ringo 配合fabric国密
                    SHA256Digest sha256Digest = new SHA256Digest();
                    byte[] retValue = new byte[sha256Digest.getDigestSize()];
                    sha256Digest.update(plainText, 0, plainText.length);
                    sha256Digest.doFinal(retValue, 0);
                    ...
                    verifier.update(retValue, 0, retValue.length);
...
//        return hash;
    ...
    public PrivateKey bytesToPrivateKey(byte[] pemKey) throws CryptoException {
        try {
            ...
            return converter.getPrivateKey(pemPair);
        }catch (Exception e){
